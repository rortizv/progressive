...
let handlerPromise: Promise<HandleWithAngular> | undefined;

/**
 * Loads the `handleWithAngular` function exported by the built Angular SSR
 * bundle (see `examples/playground-web/src/server.ts`). Cached as a
 * singleton per the AngularNodeAppEngine docs.
 */
export function getAngularHandler(
  angularServerEntry: string,
): Promise<HandleWithAngular> {
  handlerPromise ??= dynamicImport(angularServerEntry).then(
    (m) => m.handleWithAngular,
  );
